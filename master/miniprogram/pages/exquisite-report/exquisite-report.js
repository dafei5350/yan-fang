...
Page({
  ...
  onLoad: function (options) {
    this.gettoken()
    this.setData({
      'exquisite.info.orderId': options.orderId,
      'exquisite.info.masterId': options.masterId,
      _id: options._id
    })
    this.getStandard()
  },
  ...
  yesRule(e) {
    ...
    that.data.exquisite.list.push(Object.assign({}, that.data.tempYes))
    that.setData({
      'exquisite.list': that.data.exquisite.list,
      isRule: null,
  ...
  formSubmit(e) {
    ...
    if (that.data.imgList.length) {
      ...
      Promise.all(promiseArr).then(res => {
        that.data.exquisite.list.push(Object.assign({}, that.data.tempNo))
        that.setData({
          'exquisite.list': that.data.exquisite.list,
          isRule: null,
          imgList: [],
  ...
  modifyRule(e) {
    ...
    if (that.data.isRule === 'yes') {
      ...
      that.setData({
        'exquisite.list': that.data.exquisite.list.map(item => item.index === mobifyYes.index ? mobifyYes : item)
      }, () => {
        that.setData({
          activeNum: null
        })
      ...
      if (that.data.imgList.length) {
        ...
        Promise.all(promiseArr).then(res => {
          ...
          // that.data.exquisite.list = that.data.exquisite.list.map(item => item.index === mobifyNo.index ? mobifyNo : item)
          // console.log(that.data.exquisite.list);
          that.setData({
            'exquisite.list': that.data.exquisite.list.map(item => item.index === mobifyNo.index ? mobifyNo : item)
          }, () => {
            this.onClose()
          })
        })
  ...
  onReport() {
    let that = this
    console.log(that.data.exquisite.list.length);
    if (that.data.exquisite.list.length === 26) {
      that.submitReport()
    } else if (that.data.exquisite.list.length === 0) {
      wx.showModal({
        title: '上传失败',
  ...
  CopyOrderHistory() {
    let that = this
    console.log(that.data.exquisite.info.masterId);
    wx.cloud.callFunction({
      name: 'getReport',
      data: {
        type: 'CopyOrderHistory',
        _id: that.data._id,
        masterId: that.data.exquisite.info.masterId,
        finishTime: DATE
      }
    }).then(res => {
